...
# LIMITY ----
limity_cev <- read.csv("limity_cevky_mv7.csv", fileEncoding = "Windows-1250")
limity <- read.csv(
  ...
  dplyr::mutate(LIM_INDLIST = toString(na.omit(unique(LIM_INDLIST)))) %>%
  dplyr::ungroup()

indikatory_id <- read.csv("cis_indikatory_popis.csv", fileEncoding = "Windows-1250")

cis_evd_perioda <- read.csv("cis_evd_perioda.csv", fileEncoding = "Windows-1250") %>%
  dplyr::select(TAXON, PERIODA)
...
rp_code <- base::read.csv2("n2k_rp_25.csv", fileEncoding = "Windows-1250") %>%
  dplyr::rename(kod_chu = sitecode) %>%
  dplyr::select(kod_chu, pracoviste) %>%
  dplyr::mutate(pracoviste = gsub(",", "", pracoviste))

# BIOTOP EVD HMYZ ----
biotop_evd <- readr::read_csv("https://raw.githubusercontent.com/jonasgaigr/N2K.CZ/main/biotopy_evd.csv")

# NDOP EXPORT ----
n2k_export <- readr::read_csv2("C:/Users/jonas.gaigr/Documents/host_data/evl_data_export_20250408.csv", locale = readr::locale(encoding = "Windows-1250"))
